...
if (empty($fullname) || empty($id) || empty($email) || empty($phone) || empty($time)) {
    ...
    $idExistsQuery = "SELECT ID FROM Reservation WHERE ID = '$id'";
    $idExistsResult = $conn->query($idExistsQuery);

    if ($idExistsResult->num_rows > 0) {
        echo '<script>alert("Error: ID already exists. Please choose a different ID.");</script>';
        echo '<script>window.location.href = "../Includes/classes.html";</script>';
        exit();
    } else {
        ...
        echo '<script>alert("Your order has been received in the system successfully! See you soon!");</script>';
